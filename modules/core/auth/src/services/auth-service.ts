...
export class AuthService {
  ...
  private async updateSession(userId: string, oldToken: string, newToken: string) {
    const expiresAt = new Date();
    expiresAt.setHours(expiresAt.getHours() + 24);

    await this.db.$executeRaw`
      UPDATE user_sessions 
      SET token_hash = ${this.hashToken(newToken)}, expires_at = ${expiresAt}, last_accessed = NOW()
