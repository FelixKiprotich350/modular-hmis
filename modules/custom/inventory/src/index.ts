...
export async function register(ctx: ModuleContext) {
  ...
  ctx.app.use('/api/inventory', router);

  const { InventoryService } = await import('./services/inventory-service');
  ctx.registerService('InventoryService', new InventoryService(ctx.db));

  const { privileges } = await import('./privileges');
  await ctx.registerPrivileges(privileges);

  await ctx.runMigrations(path.join(__dirname, '../migrations'), 'inventory');
}
...
export async function onUninstall(ctx: ModuleContext) {
  console.log('Inventory module uninstalled');
}