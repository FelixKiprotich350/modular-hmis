...
export class AddressHierarchyController {
  ...
  async createLevel(@Body() createLevelDto: Omit<AddressHierarchyLevel, "id">) {
    const level = await this.addressHierarchyService.createLevel(
      createLevelDto
    );
    return level;
  }
  ...
  async getEntriesByLevel(@Param("levelId") levelId: string) {
    const entries = await this.addressHierarchyService.getEntriesByLevel(
      levelId
    );
  ...
  async getChildEntries(@Param("parentId") parentId: string) {
    const children = await this.addressHierarchyService.getChildEntries(
      parentId
    );
  ...
  async searchEntries(
    ...
    const entries = await this.addressHierarchyService.searchEntries(
      query,
      levelId
    );
  ...
  async getFullHierarchy(@Param("id") entryId: string) {
    const hierarchy = await this.addressHierarchyService.getFullHierarchy(
      entryId
    );
  ...
  async createEntry(@Body() createEntryDto: Omit<AddressHierarchyEntry, "id">) {
    const entry = await this.addressHierarchyService.createEntry(
      createEntryDto
    );
    return entry;
  }
  ...
  async createPersonAddress(
    @Body() createAddressDto: Omit<PersonAddress, "id" | "createdAt">
  ) {
    const address = await this.addressHierarchyService.createPersonAddress(
      createAddressDto
    );
    return address;
  }
  ...
  async getPersonAddresses(@Param("personId") personId: string) {
    const addresses = await this.addressHierarchyService.getPersonAddresses(
      personId
    );
  ...
  async updatePersonAddress(
    @Param("id") id: string,
    @Body() updateData: Partial<PersonAddress>
  ) {
    const address = await this.addressHierarchyService.updatePersonAddress(
      id,
      updateData
    );
    return address;
  ...
  async validateAddress(@Body() address: Partial<PersonAddress>) {
    const validation = await this.addressHierarchyService.validateAddress(
      address
    );
    return { validation };
  }
