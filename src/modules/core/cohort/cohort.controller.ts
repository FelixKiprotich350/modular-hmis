...
export class CohortController {
  constructor(
    @Inject("cohortService") private readonly cohortService: CohortService
  ) {}

  @Post()
  async createCohort(
    @Body() createCohortDto: Omit<Cohort, "id" | "createdAt" | "updatedAt">
  ) {
  ...
  async createCohortDefinition(
    @Body() definitionDto: Omit<CohortDefinition, "id" | "createdAt">
  ) {
    const definition = await this.cohortService.createCohortDefinition(
      definitionDto
    );
    return definition;
  }
  ...
  async evaluateDefinition(@Param("id") definitionId: string) {
    const patientIds = await this.cohortService.evaluateCohortDefinition(
      definitionId
    );
  ...
  async addPatientToCohort(
    ...
    const member = await this.cohortService.addPatientToCohort(
      cohortId,
      data.patientId
    );
    return member;
  ...
  async removePatientFromCohort(
    ...
    const removed = await this.cohortService.removePatientFromCohort(
      cohortId,
      patientId
    );
    return removed;
  ...
  async updateCohort(
    @Param("id") id: string,
    @Body() updateData: Partial<Cohort>
  ) {
