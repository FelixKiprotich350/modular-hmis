...
export class ConceptsController {
  constructor(
    @Inject("conceptService") private readonly conceptsService: ConceptService
  ) {}
  ...
  async create(@Body() createDto: CreateConceptDto) {
    const concept = await this.conceptsService.createConcept({
      ...createDto,
      retired: false,
    });
    return concept;
  ...
  async getByClass(@Param("class") conceptClass: string) {
    const concepts = await this.conceptsService.getConceptsByClass(
      conceptClass
    );
  ...
  async addAnswer(
    ...
    @Body() data: { answerConceptId: string; sortWeight?: number }
  ) {
    const answer = await this.conceptsService.addConceptAnswer(
      conceptId,
      data.answerConceptId,
      data.sortWeight
    );
    return answer;
  }
