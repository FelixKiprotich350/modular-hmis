...
export class DataExchangeController {
  ...
  @Post('import')
  async createImportJob(@Body() createImportDto: Omit<DataImport, 'id' | 'createdAt'>) {
    const job = await this.dataExchangeService.createImportJob(createImportDto);
    return { message: 'Import job created', job };
  }

  @Post('import/:id/run')
  async runImportJob(@Param('id') importId: string, @Body() data: { data: any[] }) {
    const validation = await this.dataExchangeService.validateImportData(data.data, {});
    if (!validation.valid) {
      return { success: false, errors: validation.errors };
    }
    
    const result = await this.dataExchangeService.importData(importId, data.data);
    return { message: 'Import completed', result };
  }

  @Post('import/validate')
  async validateImportData(@Body() data: { data: any[]; mappingConfig: any }) {
    const validation = await this.dataExchangeService.validateImportData(data.data, data.mappingConfig);
    return { validation };
  }

  @Post('transform')
  async transformData(@Body() data: { data: any[]; transformConfig: any }) {
    const transformedData = await this.dataExchangeService.transformData(data.data, data.transformConfig);
  ...
  async createETLJob(@Body() createETLDto: Omit<ETLJob, 'id'>) {
    ...
    return { message: 'ETL job created', job };
  }

  @Post('etl/:id/run')
  async runETLJob(@Param('id') jobId: string) {
    const success = await this.dataExchangeService.runETLJob(jobId);
    return { message: 'ETL job started', success };
  }

  @Put('etl/:id/schedule')
  async scheduleETLJob(@Param('id') jobId: string, @Body() data: { schedule: string }) {
    const job = await this.dataExchangeService.scheduleETLJob(jobId, data.schedule);
    return { message: 'ETL job scheduled', job };
  }
  ...
  @Put('jobs/:id/cancel')
  async cancelJob(@Param('id') jobId: string, @Query('type') jobType: 'export' | 'import' | 'etl') {
    const cancelled = await this.dataExchangeService.cancelJob(jobId, jobType);
    return { message: 'Job cancelled', cancelled };
  }
