...
export class DrugController {
  ...
  async createDrug(@Body() createDrugDto: Omit<Drug, 'id' | 'createdAt' | 'updatedAt'>) {
    ...
    return { message: 'Drug created', drug };
  }
  ...
  async createDrugOrder(@Body() createOrderDto: Omit<DrugOrder, 'id' | 'createdAt' | 'updatedAt'>) {
    ...
    return { message: 'Drug order created', order };
  }

  @Get('orders/patient/:patientId')
  async getPatientDrugOrders(@Param('patientId') patientId: string, @Query('status') status?: string) {
    const orders = await this.drugService.getPatientDrugOrders(patientId, status);
  ...
  async checkDrugAllergies(@Body() data: { patientId: string; drugId: string }) {
    const hasAllergy = await this.drugService.checkDrugAllergies(data.patientId, data.drugId);
    return { hasAllergy, patientId: data.patientId, drugId: data.drugId };
  }

  @Put('orders/:id/discontinue')
  async discontinueDrugOrder(@Param('id') drugOrderId: string, @Body() data: { reason: string; discontinuedBy: string }) {
    const order = await this.drugService.discontinueDrugOrder(drugOrderId, data.reason, data.discontinuedBy);
    return { message: 'Drug order discontinued', order };
  }

  @Put('orders/:id/dispense')
  async dispenseDrugOrder(@Param('id') drugOrderId: string, @Body() data: { quantityDispensed: number; dispensedBy: string }) {
    const order = await this.drugService.dispenseDrugOrder(drugOrderId, data.quantityDispensed, data.dispensedBy);
    return { message: 'Drug order dispensed', order };
  }
  ...
  async updateDrug(@Param('id') id: string, @Body() updateData: Partial<Drug>) {
    ...
    return { message: 'Drug updated', drug };
  }

  @Put('orders/:id')
  async updateDrugOrder(@Param('id') id: string, @Body() updateData: Partial<DrugOrder>) {
    const order = await this.drugService.updateDrugOrder(id, updateData);
    return { message: 'Drug order updated', order };
  }

  @Delete(':id')
  async deleteDrug(@Param('id') id: string) {
    const deleted = await this.drugService.deleteDrug(id);
    return { message: 'Drug deleted', deleted };
  }
}