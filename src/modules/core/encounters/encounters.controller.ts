...
export class EncountersController {
  constructor(
    ...
    private readonly encountersService: EncounterService
  ) {}

  @Post()
  async createEncounter(
    @Body()
    createEncounterDto: Omit<Encounter, "id" | "createdAt" | "updatedAt">
  ) {
  ...
  async getPatientEncounters(@Param("patientId") patientId: string) {
    const encounters = await this.encountersService.getPatientEncounters(
      patientId
    );
  ...
  async getProviderEncounters(@Param("providerId") providerId: string) {
    const encounters = await this.encountersService.getEncountersByProvider(
      providerId
    );
  ...
  async getEncountersByType(@Param("type") encounterType: string) {
    const encounters = await this.encountersService.getEncountersByType(
      encounterType
    );
  ...
  async getEncountersByDateRange(
    ...
    const encounters = await this.encountersService.getEncountersByDateRange(
      new Date(startDate),
      new Date(endDate)
    );
  ...
  async getEncounterWithObservations(@Param("id") id: string) {
    const encounter = await this.encountersService.getEncounterWithObservations(
      id
    );
    return { encounter };
  }
  ...
  async addObservation(
    ...
    const observation = await this.encountersService.addObservationToEncounter(
      encounterId,
      { ...obsData, obsDate: new Date() }
    );
    return observation;
  }
  ...
  async updateEncounter(
    @Param("id") id: string,
    @Body() updateData: Partial<Encounter>
  ) {
    const encounter = await this.encountersService.updateEncounter(
      id,
      updateData
    );
    return encounter;
