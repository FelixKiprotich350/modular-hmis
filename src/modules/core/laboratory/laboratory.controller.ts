...
export class LaboratoryController {
  ...
  async createLabOrder(
    @Body() createOrderDto: Omit<Laboratory, "id" | "createdAt" | "updatedAt">
  ) {
  ...
  async getPatientLabOrders(
    ...
    const orders = await this.laboratoryService.getPatientLabOrders(
      patientId,
      status
    );
  ...
  async collectSpecimen(
    ...
    @Body() data: { collectedBy: string }
  ) {
    const order = await this.laboratoryService.collectSpecimen(
      labOrderId,
      data.collectedBy
    );
    return order;
  ...
  async updateResult(
    @Param("id") id: string,
    @Body() data: { result: string; resultValue?: string; abnormal?: boolean }
  ) {
    const order = await this.laboratoryService.updateLabResult(
      id,
      data.result,
      data.resultValue,
      data.abnormal
