...
export class MetadataSharingController {
  constructor(
    @Inject("metadataSharingService")
    private readonly metadataSharingService: MetadataSharingService
  ) {}
  ...
  async exportMetadata(
    @Body() data: { types: string[]; includeRetired?: boolean }
  ) {
    const exportData = await this.metadataSharingService.exportMetadata(
      data.types,
      data.includeRetired
    );
    return { exportData };
  ...
  @Post("packages")
  async createPackage(
    @Body() createPackageDto: Omit<MetadataPackage, "id" | "dateCreated">
  ) {
    const metadataPackage = await this.metadataSharingService.createPackage(
      createPackageDto
    );
  ...
  async publishPackage(@Param("id") packageId: string) {
    const metadataPackage = await this.metadataSharingService.publishPackage(
      packageId
    );
    return metadataPackage;
  }
  ...
  async importPackage(@Body() data: { packageData: any; userId: string }) {
    const validation = await this.metadataSharingService.validatePackage(
      data.packageData
    );
    if (!validation.valid) {
    ...
    const importResult = await this.metadataSharingService.importPackage(
      data.packageData,
      data.userId
    );
    return importResult;
  ...
  @Post("sharing-config")
  async createSharingConfig(
    @Body() configDto: Omit<MetadataSharing, "id" | "createdAt">
  ) {
    const config = await this.metadataSharingService.createSharingConfiguration(
      configDto
    );
    return config;
  }
  ...
  async getSharingConfigurations() {
    const configurations =
      await this.metadataSharingService.getSharingConfigurations();
  ...
  async updatePackage(
    @Param("id") id: string,
    @Body() updateData: Partial<MetadataPackage>
  ) {
    const metadataPackage = await this.metadataSharingService.updatePackage(
      id,
      updateData
    );
    return metadataPackage;
