...
export class ObservationsController {
  ...
  async createObservation(
    @Body() createObsDto: Omit<Observation, "id" | "createdAt" | "updatedAt">
  ) {
    const observation = await this.observationsService.createObservation(
      createObsDto
    );
    return observation;
  }
  ...
  async createObservationGroup(
    ...
    const group = await this.observationsService.createObservationGroup(
      data.patientId,
      data.encounterId,
      data.conceptId,
      data.observations
  ...
  async recordVitalSigns(
    ...
    data: {
      patientId: string;
      encounterId: string;
      vitals: { conceptId: string; value: string; units?: string }[];
    }
  ) {
    const observations = await this.observationsService.recordVitalSigns(
      data.patientId,
      data.encounterId,
      data.vitals
    );
  ...
  async getPatientObservations(
    ...
    @Query("concept") conceptId?: string
  ) {
    const observations = await this.observationsService.getPatientObservations(
      patientId,
      conceptId
    );
  ...
  async getPatientVitalSigns(
    ...
    const vitals = await this.observationsService.getVitalSigns(
      patientId,
      encounterId
    );
  ...
  async getLatestObservation(
    ...
    const observation = await this.observationsService.getLatestObservation(
      patientId,
      conceptId
    );
  ...
  async getEncounterObservations(@Param("encounterId") encounterId: string) {
    const observations =
      await this.observationsService.getEncounterObservations(encounterId);
  ...
  async getObservationsByConcept(@Param("conceptId") conceptId: string) {
    const observations =
      await this.observationsService.getObservationsByConcept(conceptId);
    return { observations, conceptId };
  }

  @Get("patient/:patientId/by-date")
  async getObservationsByDateRange(
    @Param("patientId") patientId: string,
    @Query("start") startDate: string,
    @Query("end") endDate: string
  ) {
    const observations =
      await this.observationsService.getObservationsByDateRange(
        patientId,
        new Date(startDate),
        new Date(endDate)
      );
  ...
  async updateObservation(
    @Param("id") id: string,
    @Body() updateData: Partial<Observation>
  ) {
    const observation = await this.observationsService.updateObservation(
      id,
      updateData
    );
    return observation;
  ...
  async voidObservation(
    ...
    const observation = await this.observationsService.voidObservation(
      id,
      data.reason
    );
    return observation;
