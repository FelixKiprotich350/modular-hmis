...
export class OrderController {
  ...
  async getPatientOrders(@Param('patientId') patientId: string, @Query('status') status?: string) {
  ...
  @Get('by-date')
  async getOrdersByDateRange(@Query('start') startDate: string, @Query('end') endDate: string) {
    const orders = await this.orderService.getOrdersByDateRange(new Date(startDate), new Date(endDate));
  ...
  async activateOrder(@Param('id') orderId: string) {
    ...
    return { message: 'Order activated', order };
  }

  @Put(':id/discontinue')
  async discontinueOrder(@Param('id') orderId: string, @Body() data: { reason: string; discontinuedBy: string }) {
    const order = await this.orderService.discontinueOrder(orderId, data.reason, data.discontinuedBy);
    return { message: 'Order discontinued', order };
  }

  @Put(':id/renew')
  async renewOrder(@Param('id') orderId: string, @Body() data: { newAutoExpireDate?: Date }) {
    const order = await this.orderService.renewOrder(orderId, data.newAutoExpireDate);
    return { message: 'Order renewed', order };
  }
  ...
  async deleteOrder(@Param('id') id: string) {
    ...
    return { message: 'Order deleted', deleted };
  }
}