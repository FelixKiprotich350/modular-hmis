...
export class VisitsController {
  constructor(
    @Inject("visitService") private readonly visitsService: VisitService
  ) {}

  @Post()
  async createVisit(
    @Body() createVisitDto: Omit<Visit, "id" | "createdAt" | "updatedAt">
  ) {
  ...
  @Post("start")
  async startVisit(
    @Body() data: { patientId: string; visitType: string; notes?: string }
  ) {
  ...
  async getVisitsByDateRange(
    ...
    const visits = await this.visitsService.getVisitsByDateRange(
      new Date(startDate),
      new Date(endDate)
    );
  ...
  async addEncounterToVisit(
    ...
    const encounter = await this.visitsService.addEncounterToVisit(visitId, {
      ...encounterData,
      startDate: new Date(),
    });
    return encounter;
